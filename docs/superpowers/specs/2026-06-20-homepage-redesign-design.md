...
- **形式**：案例卡片网格（4 列桌面 / 2 列平板 / 1 列移动），每张卡含项目信息
- **4 个典型案例**（用户提供真实信息）：

| 案例 | 项目类型 | 客户需求 | 服务内容 | 认证结果 |
|------|----------|----------|----------|----------|
| 案例 1 | LED 灯具出口欧盟 | CE+RoHS 认证 | 安规+EMC+光性能+化学 | 获 TUV-Mark |
| 案例 2 | 数控机床出口北美 | CE+RoHS 认证 | 机械安全+EMC | 获 UL 认证 |
| 案例 3 | 玩具出口欧盟 | EN 71 全项 | 物理+燃烧+化学 | 获 CE 认证 |
| 案例 4 | 建材出口中东 | EN 71 全项 | 物理性能+化学 | 顺利清关 |

- **客户评价**（独立展示区，来自宣传手册 OCR，待用户提供真实对应关系）：
...
> 注：以上评价来自宣传手册 OCR，与案例的对应关系待用户确认。提供真实对应关系后可调整为一一对应展示。
...
| 典型案例 | 用户提供真实信息（4 个案例） | 真实值 |
| 客户评价 | 宣传手册 OCR | 待确认对应关系 |
...
case_4_type, case_4_need, case_4_service, case_4_result
...
9. **典型案例 4 个**：案例信息由用户提供真实值（LED 灯具/数控机床/玩具/建材），客户评价来自 OCR 待确认对应关系
